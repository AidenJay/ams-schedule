    ...
    public void start() {
        logrThread = new Thread(new Runnable() {
            ...
            public void run() {
                    ...
                    try {
                        ...
                        for (int i = 0; i < 3; i++) {
                            ...
                            if (triggerCountMap != null && triggerCountMap.size() > 0) {
                                int triggerDayCount = triggerCountMap.containsKey("triggerDayCount") ? Integer.valueOf(
                                        String.valueOf(triggerCountMap.get("triggerDayCount"))) : 0;
                                int triggerDayCountRunning = triggerCountMap.containsKey(
                                        "triggerDayCountRunning") ? Integer.valueOf(
                                        String.valueOf(triggerCountMap.get("triggerDayCountRunning"))) : 0;
                                int triggerDayCountSuc = triggerCountMap.containsKey(
                                        "triggerDayCountSuc") ? Integer.valueOf(
                        ...
                        if (!toStop) {
                            logger.error(">>>>>>>>>>> schedule-job, job log report thread error:{}", e);
                        }
                    }
                ...
                logger.info(">>>>>>>>>>> schedule-job, job log report thread stop");

            }
        });
        logrThread.setDaemon(true);
        logrThread.setName("schedule-job, admin JobLogReportHelper");
        logrThread.start();
    }
