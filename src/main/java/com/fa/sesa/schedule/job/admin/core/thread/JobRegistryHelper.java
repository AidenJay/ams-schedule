    ...
    public void start() {
        ...
        registryOrRemoveThreadPool = new ThreadPoolExecutor(2, 10, 30L, TimeUnit.SECONDS,
            ...
            public void rejectedExecution(Runnable r, ThreadPoolExecutor executor) {
                r.run();
                logger.warn(
                        ">>>>>>>>>>> schedule-job, registry or remove too fast, match threadpool rejected handler(run" +
                                " now).");
            }
        });
            ...
            public void run() {
                while (!toStop) {
                    try {
                        ...
                        if (!toStop) {
                            logger.error(">>>>>>>>>>> schedule-job, job registry monitor thread error:{}", e);
                        }
                    }
                    try {
                ...
                logger.info(">>>>>>>>>>> schedule-job, job registry monitor thread stop");
            }
        });
        registryMonitorThread.setDaemon(true);
        registryMonitorThread.setName("schedule-job, admin JobRegistryMonitorHelper-registryMonitorThread");
