    ...
    public void start() {
        ...
        callbackThreadPool = new ThreadPoolExecutor(2, 20, 30L, TimeUnit.SECONDS,
            ...
            public Thread newThread(Runnable r) {
                return new Thread(r, "schedule-job, admin JobLosedMonitorHelper-callbackThreadPool-" + r.hashCode());
            }
        }, new RejectedExecutionHandler() {
            @Override
            public void rejectedExecution(Runnable r, ThreadPoolExecutor executor) {
                r.run();
                logger.warn(">>>>>>>>>>> schedule-job, callback too fast, match threadpool rejected handler(run now).");
            }
        });
            ...
            public void run() {
                ...
                while (!toStop) {
                    try {
                        ...
                        if (!toStop) {
                            logger.error(">>>>>>>>>>> schedule-job, job fail monitor thread error:{}", e);
                        }
                    }
                ...
                logger.info(">>>>>>>>>>> schedule-job, JobLosedMonitorHelper stop");

            }
        });
        monitorThread.setDaemon(true);
        monitorThread.setName("schedule-job, admin JobLosedMonitorHelper");
        monitorThread.start();
    }
