                    ...
                    <div class="input-group">
                        <input type="text" class="form-control" id="author"
                               placeholder="${I18n.system_please_input}${I18n.jobinfo_field_author}"
                                <#if Request["XXL_JOB_LOGIN_IDENTITY"]?? && Request["XXL_JOB_LOGIN_IDENTITY"].role == 0>
                                    value="${Request["XXL_JOB_LOGIN_IDENTITY"].username}"
                                    readonly="readonly"
                                </#if>
                        >
...
public class DemoGlueJobHandler extends AmsCustomJob {

    @Override
    public void execute() throws Exception {
        String sql = "SELECT PROCESS_LOCATION,count(*) LOTHOLDCOUNT FROM lot WHERE LOT_STATUS='HOLD' AND "
                     + "PROCESS_LOCATION IS NOT NULL GROUP BY PROCESS_LOCATION";

        executeSql(sql, "AMS告警源", "邮件标题", "邮件内容");
    }
}
</textarea>
